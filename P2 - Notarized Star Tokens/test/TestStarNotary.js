...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.name(), "Udacity Wonder Token");
    assert.equal(await instance.symbol(), "UWT");
});
...
it('lets 2 users exchange stars', async() => {
    ...
    let newStar1 = await instance.tokenIdToStarInfo.call(tokenId1);
    let owner1 = await instance.ownerOf(tokenId1);
    await instance.createStar('Great Star!', tokenId2, {from: accounts[1]})
    let newStar2 = await instance.tokenIdToStarInfo.call(tokenId2);
    let owner2 = await instance.ownerOf(tokenId2);

    // 2. Call the exchangeStars functions implemented in the Smart Contract
    await instance.exchangeStars(tokenId1, tokenId2, {from: accounts[0], to: accounts[1]});
...
it('lets a user transfer a star', async() => {
    ...
    await instance.transferStar(accounts[1], tokenId, {from: accounts[0], to: accounts[1]});
