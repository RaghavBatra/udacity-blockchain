...
contract Ownable {
    ...
    constructor() internal {
      _owner = msg.sender;
      emit transferOfOwnership(address(0), _owner);
    }
    //  3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
    modifier onlyOwner(address _account) {
    require(_owner == _account, "Sender not owner");
    _;
    }
    ...
    event transferOfOwnership(address oldOwner, address newOwner);
...
contract Pausable is Ownable {
  ...
  function setPauseMode (bool newMode) public onlyOwner(msg.sender) {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(ownerOf(tokenId) != to, "Recipient is already owner!");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "Is not approved");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
      ...
      string memory concatenatedString = strConcat(_baseTokenURI, uint2str(tokenId));

      _tokenURIs[tokenId] = concatenatedString;
...
contract ERC721Mintable is ERC721Metadata {
  ...
  public {
    ...
    function mint(address to, uint256 tokenId) onlyOwner(msg.sender) public returns (bool) {
